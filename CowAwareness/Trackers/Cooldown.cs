namespace CowAwareness.Trackers
{
    ...
    public class Cooldown : Feature
    {
        ...
        {
            get { return this["enemies"].Cast<CheckBox>().CurrentValue; }
        }

        private bool TrackAllies
        {
            get { return this["allies"].Cast<CheckBox>().CurrentValue; }
        }

        private bool TrackSelf
        {
            get { return this["self"].Cast<CheckBox>().CurrentValue; }
        }
        ...
        protected override void Initialize()
        {
            ...
            SummonerSpells.Add("summonerteleport", new Sprite(TextureLoader.BitmapToTexture(Resources.summonerteleport)));
            ...
            SummonerSpells.Add("s5_summonersmiteduel", new Sprite(TextureLoader.BitmapToTexture(Resources.s5_summonersmiteduel)));
            SummonerSpells.Add("s5_summonersmiteplayerganker", new Sprite(TextureLoader.BitmapToTexture(Resources.s5_summonersmiteplayerganker)));
        ...
        private void DrawSummoner(AIHeroClient hero, SpellSlot slot)
        {
            ...
            float cooldown = spell.CooldownExpires - Game.Time;
            Vector2 location = this.GetSummonerLocation(hero, spell.Slot);
            Color color = cooldown > 0 ? Color.Red : Color.Green;
            ...
            float lineX = location.X + (hero.IsMe ? 17 : -3);
            Drawing.DrawLine(new Vector2(lineX, location.Y), new Vector2(lineX, location.Y - 16), 3f, color);

            if (cooldown > 0)
            {
                var offset = hero.IsMe ? 22 : -27;
                this.text.Draw(string.Format("{0,3}", (int)Math.Ceiling(cooldown)), Color.White, (int)location.X + offset, (int)location.Y);
            }
        }
        ...
        {
            int gap = 27;
            float x = hero.HPBarPosition.X + (hero.IsMe ? 34 : 0) + this.AdditionalXOffset(hero);
            float y = hero.HPBarPosition.Y + 23;
        ...
        {
            float x = hero.HPBarPosition.X + (hero.IsMe ? 133 : -18) + this.AdditionalXOffset(hero);
            float y = hero.HPBarPosition.Y + (hero.IsMe ? -9 : -4);
        ...
        {
            string champName = hero.ChampionName.ToLower();
